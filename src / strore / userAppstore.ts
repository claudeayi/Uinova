...
interface AppState {
  ...
  toggleEditorGrid: (value: boolean) => void;
}
    ...
    (set, get) => ({
      ...
      emitElements: (elements) => {
        try {
          ...
          console.error("emitElements error:", e);
        }
      },
      ...
      listenElements: () => {
        ...
        socket.on("usersCount", (count: number) =>
          set({ onlineUsers: count, isOnline: true })
        );
      },
      ...
      importProject: (json) => {
        try {
          ...
          console.error("Erreur import project:", e);
        }
      },
      ...
      preferences: {
        ...
        editorGrid: localStorage.getItem("editorGrid") === "true",
      },
      ...
      toggleEditorGrid: (value) => {
        localStorage.setItem("editorGrid", String(value));
        set((state) => ({
          preferences: { ...state.preferences, editorGrid: value },
        }));
      },
    }),
    { name: "uinova-store" }
