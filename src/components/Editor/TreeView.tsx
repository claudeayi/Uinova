...
type Props = {
  ...
  onReorder?: (next: ElementData[]) => void; // optionnel: si fourni, on renvoie l’arbre réordonné
};
...
export default function TreeView({ elements, onSelect, onReorder }: Props) {
  const sensors = useSensors(useSensor(PointerSensor, { activationConstraint: { distance: 4 } }));

  const ids = useMemo(() => flatten(elements).map((n) => n.el.id), [elements]);

  function byId(id: string): { el: ElementData; path: number[] } | null {
    return flatten(elements).find((n) => n.el.id === id) || null;
  }
  ...
  const handleDragEnd = (e: DragEndEvent) => {
    if (!onReorder) return; // si pas de callback, on garde la vue read-only
    const { active, over } = e;
    ...
    // Simplification : on réordonne au même niveau (move entre frères)
    const [removed, treeWithout] = removeAtPath(elements, a.path);

    // dépose avant la position de b (même parent)
